...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    while cap.isOpened():
        ...
        if len(sequence) == 30 and total !=0:
            ...
            print(signs[position])
            predictions.append(position)

            uni,uniPos=np.unique(predictions[-7:], return_counts=True)
            
            if max(uniPos) > 4  and uni[np.argmax(uniPos)]==position:  #max of last 10 frames must be same word
                if res[position] > threshold: 
                    
                    predicted_text = signs[position]
            ...
            sequence = sequence[-25:]               #to slow down predicting by losing first few sequence

        total=0
            
        cv2.rectangle(image, (0,0), (640, 40), (245, 117, 16), -1)
        cv2.rectangle(image, (0,440), (640, 480), (0, 0, 255), -1)

        cv2.putText(image,'Your:- ' + speech, (3,30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
        cv2.putText(image,'Speaker:- ' + speakers_speech, (3,470), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
